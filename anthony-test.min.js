...
{volume:b,ad_duration:d})}D.push(f);if(b=p[f])for(key in b){"AdStopped"==f&&console.log("Stopped Key: "+key);var e=b[key];e&&(c("found callback for "+f),(y||z)&&"AdStopped"==f&&e==E?setTimeout(function(){console.log("called stop callback (with delay) "+key);e()},1E3):("AdStopped"==f&&console.log("called stop callback (no delay) "+key),e()))}}var g=null,n=null,k=null,p={},d={},u=-1,v=-1,B=-1,x=null,r=null,A=null,H=this,D=[],C=!1,z=!1,y=!1,E=null;b("https://imasdk.googleapis.com/js/sdkloader/ima3.js");
...
function(a,b){c("unsubscribe "+b);p[b][a]=null};this.subscribe=function(a,b,d){c("Subscribe for "+b+" - "+a);d=d?a.bind(d):a;var e=p[b];"AdStopped"==b&&console.log("REGISTERING "+a);e||("AdStopped"==b&&(E=d),e=p[b]={});e[a]=d};this.expandAd=function(){c("expandAd (not supported)")};this.collapseAd=function(){c("collapseAd (not supported)")};this.skipAd=function(){c("skipAd");t()&&g.O()};this.resumeAd=function(){c("resumeAd");g.M()};this.pauseAd=function(){c("pauseAd");g.J()};this.resizeAd=function(b,
